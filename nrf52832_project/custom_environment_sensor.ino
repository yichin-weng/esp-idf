...
void setup()
{
  ...
  Serial.println("Bluefruit52 ENVS Example");
  Serial.println("-----------------------\n");
...
void readTrim()
{
    ...
    Wire.write(0x88);                   // read reg address 0x88
    Wire.endTransmission();
    Wire.requestFrom(BME280_addr,24);   // read 24 bytes data
    ...
    Wire.beginTransmission(BME280_addr);
    Wire.write(0xA1);                   // read reg address 0xA1
    Wire.endTransmission();
    Wire.requestFrom(BME280_addr,1);    // read 1 byte
    data[i] = Wire.read();
    i++;

    Wire.beginTransmission(BME280_addr);
    Wire.write(0xE1);                   // read reg address 0xE1
    Wire.endTransmission();
    Wire.requestFrom(BME280_addr,7);    // read 7 bytes of data
    while(Wire.available()){
        data[i] = Wire.read();
        i++;
    }
