...
static void IRAM_ATTR timer_cb(void *para){
...
static void example_timer_init(int timer_idx, bool auto_reload, double timer_interval_sec) {
            ...
            if (param->status_cb.sensor_status.marshalled_sensor_data->len) {
                ...
                for (; length < param->status_cb.sensor_status.marshalled_sensor_data->len; ) {
                    ...
                    if (data_len != ESP_BLE_MESH_SENSOR_DATA_ZERO_LEN) {
                        ...
                        if (prop_id == 0x0056) {
                            memcpy(my_temp.temp_data, data + mpid_len, 8);
                        } else if (prop_id == 0x005b) {
                            memcpy(my_hum.hum_data, data + mpid_len, 8);
                        } else if (prop_id == 0x0060) {
                            memcpy(my_press.press_data, data + mpid_len, 8);
                        } else {
                            ;
                        }
...
void app_main(void)
{
    ...
    example_timer_init(TIMER_0, TEST_WITHOUT_RELOAD, TIMER_INTERVAL0_SEC);
